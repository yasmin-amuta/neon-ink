document.addEventListener("DOMContentLoaded", (event) => {
  ...
  footerTl
    ...
    .from(".footer-left", { xPercent: -20, duration: 1 }, "<")
    .from(".footer-right form", { xPercent: 20, duration: 1 }, "<");

  // 1. Animate each SECTION HEAD
  document.querySelectorAll("[data-section]").forEach((section) => {
    const head = section.querySelector("[data-head]");
    const image = section.querySelectorAll(".prod-img");

    // FIX: Ensure all subtexts start hidden
    gsap.set(image, { opacity: 0, x: -70 });

    const productSection = gsap.timeline({
    ...
    productSection
      ...
      // Timeline for subtitles under each image
      // Subtexts animation
      .to(
        image,
        {
